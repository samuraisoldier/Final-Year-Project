            ...
            if meas['element_type'][k] == 'line':
                ...
                for i in range(no_buses):
                    ...
                    if i == 0:
                        ...
                        ans = 1000 * vi * vj * (gij*np.sin(thetaij) - bij*np.cos(thetaij))
                        row.append(ans)
                    else:
                        row.append(0)
                #do voltages
                ...
                thetaij=thetai-thetaj
                bij=lines.loc[lines['from_bus'] == ind1].reset_index(drop = True).loc[lines['to_bus'] == ind2].reset_index(drop = True)['X'][0]
                ans1 = 1000 * (-1*vj * (gij*np.cos(thetaij) - bij*np.sin(thetaij)) + 2 * gij * vi)
                ans2 = 1000 * (-1*vi * (gij*np.cos(thetaij) - bij*np.sin(thetaij)))
                ansrow = [0]*3
                ansrow[ind1-1] = ans1
                ansrow[ind2-1] = ans2
                row = row + ansrow
                ...
                for j in range(no_buses):
                    ...
                    thetaij=thetai-thetaj
                    Bij=ad_row[j].imag
                    ans = ans + (vi * vj * (-Gij*np.sin(thetaij) + Bij*np.cos(thetaij)))
                row[0]=round(ans-vi * ad_row[1].imag,2)
                thetaij = x_state[1]-x_state[0]
                Gij=ad_row[2].real
                Bij=ad_row[2].imag
                row[1] = vi * x_state[4]*(Gij*np.sin(thetaij)-Bij*np.cos(thetaij))
                ...
                for i in range(no_buses):
                    if i == 1:
                        ...
                        for j in range(no_buses):
                            ...
                            thetaij=thetai-thetaj
                            Bij=ad_row[j].imag
                            ans = ans + (vj * (-Gij*np.sin(thetaij) + Bij*np.cos(thetaij)))
                        row[3] = ans + vi*ad_row[i].real
                    else:
                        if i == 0:
                            thetaj = 0
                        else:
                        ...
                        Gij=ad_row[i].real
                        Bij=ad_row[i].imag
                        row[i+2] = vi * (Gij*np.cos(thetaij)+Bij*np.sin(thetaij))
                h_jaco.append(row)
            ...
            if meas['element_type'][k] == 'line':
                ...
                for i in range(no_buses):
                    ...
                    if i == 0:
                        ...
                        thetaij=thetai-thetaj
                        bij=lines.loc[lines['from_bus'] == ind1].reset_index(drop = True).loc[lines['to_bus'] ==ind2].reset_index(drop = True)['X'][0]
                        ans =1000* vi * vj * (gij*np.cos(thetaij) + bij*np.sin(thetaij))
                        row.append(ans)
                    else:
                        row.append(0)
                #do voltages
                ...
                thetaij=thetai-thetaj
                bij=lines.loc[lines['from_bus'] == ind1].reset_index(drop = True).loc[lines['to_bus'] == ind2].reset_index(drop = True)['X'][0]
                ans1 = 1000 * (-vj * (gij*np.sin(thetaij) - bij*np.cos(thetaij)) - 2 * bij * vi)
                ans2 = 1000 * (-vi * (gij*np.sin(thetaij) - bij*np.cos(thetaij)))
                ansrow = [0]*3
                ...
                for j in range(no_buses):
                    ...
                    thetaij=thetai-thetaj
                    Bij=ad_row[j].imag
                    ans = ans + (vi * vj * (Gij*np.cos(thetaij) + Bij*np.sin(thetaij)))
                row[0]=round(ans-vi *vi* ad_row[1].real,2)
                thetaij = x_state[1]-x_state[0]
                Gij=ad_row[2].real
                Bij=ad_row[2].imag
                row[1] = vi * x_state[4]*(-Gij*np.cos(thetaij)-Bij*np.sin(thetaij))
                ...
                for i in range(no_buses):
                    if i == 1:
                        ...
                        Gij=ad_row[i].real
                        Bij=ad_row[i].imag
                        row[i+2] = vi * (Gij*np.sin(thetaij)-Bij*np.cos(thetaij))
                h_jaco.append(row)
    ...
    for k in range(no_meas):
        if meas['measurement_type'][k] == 'p':
            if meas['element_type'][k] == 'line':
                ...
                thetaj=x_state[ind2-2]
                thetaij=thetai-thetaj
                h_meas[k] =  vi**2 * gij - (vi * vj * (gij*np.cos(thetaij) + bij*np.sin(thetaij)))
                ...
                for j in range(no_buses):
                    ...
                    if j == 0:
                        ...
                        thetaj= x_state[j-1]
                    thetaij=thetai-thetaj
                    h_meas[k] += (vj * (Gij*np.cos(thetaij) + Bij*np.sin(thetaij)))
                h_meas[k] = vi*h_meas[k]
        elif meas['measurement_type'][k] == 'q':
            if meas['element_type'][k] == 'line':
                buses = literal_eval(meas['buses_tf'][k])
                ind1 = buses[0]
                ...
                thetaj=x_state[ind2-2]
                thetaij=thetai-thetaj
                h_meas[k] =  -vi**2 * bij - (vi * vj * (gij*np.sin(thetaij) - bij*np.cos(thetaij)))
            elif meas['element_type'][k] == 'bus':
                bus = meas['element'][k]
                vi=x_state[bus+1]
                thetai=x_state[bus-2]
                ...
                for j in range(no_buses):
                    ...
                    if j == 0:
                        ...
                        thetaj= x_state[j-1]
                    thetaij=thetai-thetaj
                    h_meas[k]  += (vj * (Gij*np.sin(thetaij) - Bij*np.cos(thetaij)))
                h_meas[k] = vi*h_meas[k]
        elif meas['measurement_type'][k] == 'v':
            bus = meas['element'][k]
            ans = x_state[bus+1]
